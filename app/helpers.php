...
{
    static $display;
    isset($display) || $display = apply_filters('uvigothemewp/display_sidebar', false);
    return $display;
}
 ...
 * @return string
 */
function display_date($datestringiso)
{
    if (isset($datestringiso) && !empty($datestringiso)) {
        $time = date_create_from_format(DATE_ISO8601, $datestringiso);
        return date_i18n(get_option('date_format'), $time->getTimestamp());
    }
    return '';
}
